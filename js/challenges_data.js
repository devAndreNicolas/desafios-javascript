...
export const challenges = [
  {
    ...
    title: "Nossa Equipe",
    description: `
    Conheça nossa equipe de desenvolvedores presentes no projeto.
    `,
    difficulty: "team",
    inputs: [],
    solution: function () {
  ...
  - Caso contrário, some todos os números e divida pela quantidade de números.`,
    ...
    solution: function (inputs) {
      try {
        ...
        const arrNumeros = strNumeros
          .split(",")
          .map((s) => s.trim())
          .map(Number);
        if (arrNumeros.some(isNaN))
          return "Erro: entrada inválida. Use apenas números separados por vírgula.";
  ...
  },
  {
    ...
    description:
      'Dada uma entrada numérica n, gere uma string recursivamente concatenando "chunk" com hífens. Para n=1, retorna "chunk". Para n>1, retorna "chunk-" seguido do resultado para n-1.',
    difficulty: "very-easy",
    inputs: [{ name: "n", type: "number", label: "Digite o valor de n:" }],
    solution: function (inputs) {
    ...
    inputs: [
      {
        name: "array",
        type: "text",
        label: "Digite um array separado por vírgulas:",
      },
    ],
    solution: function (inputs) {
  ...
  },
  {
    ...
    description:
      "Para cada dígito do número, calcula o quadrado e concatena os resultados.",
    difficulty: "very-easy",
    inputs: [{ name: "numero", type: "number", label: "Digite um número:" }],
    solution: function (inputs) {
  ...
  },
  {
    ...
    description:
      "Dada uma frase, retorne a maior letra (a-z) lexicograficamente, ignorando caracteres não alfabéticos.",
    difficulty: "very-easy",
    inputs: [{ name: "frase", type: "text", label: "Digite uma frase:" }],
    solution: function (inputs) {
  ...
  },
  {
    ...
    description:
      "Dada uma frase, inverta cada palavra individualmente, removendo acentos e caracteres especiais, e retorne a frase resultante em letras minúsculas.",
    difficulty: "very-easy",
    inputs: [{ name: "frase", type: "text", label: "Digite uma frase:" }],
    solution: function (inputs) {
      let fraseLimpa = inputs.frase
        .normalize("NFD")
        .replace(/[^a-zA-Z\s]/g, "")
        .replace(/\s+/g, " ")
        .trim();
  ...
  ...Array.from({}, (_, i) => ({
    ...
    solution: function (inputs) {
      return `Resultado para Muito Fácil ${i + 1}: ${
        inputs[`inputVE${i}`] || "vazio"
      }`;
    },
  })),
  ...
  // --- EASY ---
  {
    ...
    description:
      "Dado um array bidimensional contendo números, transforme-o em um array unidimensional com todos os números ordenados em ordem crescente.",
    difficulty: "easy",
    inputs: [
      {
        name: "matriz",
        type: "text",
        label: "Digite a matriz (ex: [[3,5,1],[8,2],[7,4,6]]):",
      },
    ],
    solution: function (inputs) {
      try {
        const arrayBidimensional = JSON.parse(inputs.matriz);
        if (!Array.isArray(arrayBidimensional))
          return "Erro: A entrada não é uma matriz válida.";
        const resultado = [];
...

  {
    ...
    description:
      'Dado um nome completo, retorne a abreviação do primeiro(s) nome(s) seguido do sobrenome completo em letras maiúsculas. Exemplo: "Ruan Wallice da silva lima" → "R. W. D. S., LIMA".',
    difficulty: "easy",
    inputs: [
    ...
    solution: function (inputs) {
      try {
        ...
        if (nomes.length < 2)
          return "Erro: Informe pelo menos nome e sobrenome.";
        const ultimoNome = nomes.pop().toUpperCase();
        const abreviaturas = nomes
...

  {
    ...
    description:
      'Verifique se todos os caracteres de uma string aparecem a mesma quantidade de vezes. Por exemplo, "aabb" retorna true, "aabbc" retorna false.',
    difficulty: "easy",
    inputs: [
...

  {
    ...
    description:
      "Dado um número inteiro não negativo, calcule seu fatorial. O fatorial de n (n!) é o produto de todos os números inteiros positivos menores ou iguais a n.",
    difficulty: "easy",
    inputs: [
      {
        name: "numero",
        type: "number",
        label: "Digite um número inteiro não negativo:",
      },
    ],
    solution: function (inputs) {
      try {
...

  {
    ...
    description:
      "Dado dois números inteiros positivos representando colunas (x) e linhas (y), gere todos os pares de coordenadas no formato [x, y], percorrendo da esquerda para a direita e de cima para baixo.",
    difficulty: "easy",
    inputs: [
    ...
    inputs: [
      {
        name: "codigo",
        type: "text",
        label: "Digite o código da nave (12 dígitos):",
      },
    ],
    solution: function (inputs) {
      try {
  ...
  ...Array.from({}, (_, i) => ({
    ...
    solution: function (inputs) {
      return `Resultado para Desafio Fácil ${i + 7}: ${
        inputs[`inputE${i}`] || "vazio"
      }`;
    },
  })),
...
- \`relatorio("usar cápsulas emergência", "de ", [5, 20])\` → "usar de cápsulas de emergência"`,
    ...
    inputs: [
      { name: "frase", type: "text", label: "Digite a frase original:" },
      {
        name: "palavra",
        type: "text",
        label: "Digite a palavra a ser inserida:",
      },
      {
        name: "indices",
        type: "text",
        label: "Digite os índices de inserção (separados por vírgula):",
      },
    ],
    solution: function (inputs) {
      ...
      indices.forEach((indice) => {
        if (indice >= 0 && indice <= resultado.length) {
          resultado =
            resultado.slice(0, indice) + palavra + resultado.slice(indice);
        }
      });
...
- \`persistenciaMultiplicativa(4)\` → 0 (já é um único dígito)`,
    difficulty: "medium",
    inputs: [{ name: "numero", type: "number", label: "Digite o número:" }],
    solution: function (inputs) {
    ...
    inputs: [
      {
        name: "array",
        type: "text",
        label: "Digite os elementos do array (separados por vírgula):",
      },
    ],
    solution: function (inputs) {
...
- Saída: ["0003"]`,
    ...
    inputs: [
      {
        name: "lista",
        type: "text",
        label: "Digite os códigos dos prisioneiros (separados por vírgula):",
      },
    ],
    solution: function (inputs) {
  ...
  ...Array.from({}, (_, i) => ({
    ...
    solution: function (inputs) {
      return `Resultado para Desafio Médio ${i + 4}: ${
        inputs[`inputM${i}`] || "vazio"
      }`;
    },
  })),
  // --- HARD ---
  {
    ...
    description: `Este desafio atualiza a classificação de veículos com base em comandos. ...`,
    difficulty: "hard",
    inputs: [
      {
        name: "classificacao",
        type: "textarea",
        label:
          'Digite a classificação inicial (ex: ["Fusca", "Kombi", "Marea", "Brasília"]):',
      },
      {
        name: "comando",
        type: "text",
        label: 'Digite o comando (ex: "Kombi +1"):',
      },
    ],
    solution: function (inputs) {
      try {
        // Se classificacaoState for nulo, inicialize-o com o JSON de entrada
        if (!window.classificacaoState) {
          window.classificacaoState = JSON.parse(inputs.classificacao);
        }

        const comando = inputs.comando.trim();
        const [nome, acao] = comando.split(" ");
        let index = window.classificacaoState.findIndex((c) => c.startsWith(nome));

        if (index === -1)
          return "Erro: Veículo não encontrado na classificação.";

        const isEliminated = window.classificacaoState[index].includes("ELIMINATED");

        if (acao === "ELIMINATE") {
          if (!isEliminated) {
            const eliminado =
              window.classificacaoState.splice(index, 1)[0] + " ELIMINATED";
            window.classificacaoState.push(eliminado);
          }
        } else {
          const deslocamento = parseInt(acao);
          if (!isEliminated && !isNaN(deslocamento)) {
            const [carro] = window.classificacaoState.splice(index, 1);
            const novoIndex = Math.max(
              0,
              Math.min(window.classificacaoState.length, index - deslocamento)
            );
            window.classificacaoState.splice(novoIndex, 0, carro);
          }
        }

        return window.classificacaoState;
      } catch (e) {
...
- [["123", "Conexão A"], ["456", "Conexão B"]]`,
      ...
      try {
        ...
        function buscarConexoes(obj) {
          ...
          if (obj.connection && obj.connection._id && obj.connection.label) {
            conexoesEncontradas.push([
              obj.connection._id,
              obj.connection.label,
            ]);
          }
  ...
  },
  {
    ...
    Embarque em uma emocionante jornada de exploração espacial! Neste desafio, você será um destemido explorador, desbravando planetas desconhecidos e registrando suas descobertas.

Como funciona:

1.  Você fornece os dados iniciais do explorador (nível, experiência, etc.) em formato JSON.
2.  Você escolhe um planeta para explorar, também em JSON, com suas características (terreno, recursos, etc.).
3.  Você lança dois dados (números) para simular o resultado da exploração.
4.  O programa usa esses dados para atualizar as informações do explorador (experiência, nível, planetas conhecidos, etc.).
...

{
    "name": "Veridia",
    "terrain": "floresta",
    "resources": ["plantas medicinais", "frutas exóticas"]
}

*   dado1: 5
*   dado2: 3

Nesse caso, o explorador está no nível 1, sem experiência, é um "Novato", não conhece planetas, está vivo e pode explorar. Ele está indo para o planeta "Veridia", que é uma floresta.

O programa vai somar os dados (5 + 3 = 8) e atualizar o explorador. Como a soma não é 2 (1+1), ele ganha 25 de experiência e adiciona "Veridia" à lista de planetas conhecidos.
...
Experiência: 25
Ranque: Novato
Planetas Conhecidos: Veridia
Vivo: true
Pode Explorar: true

Se os dados fossem 1 e 1, o explorador morreria (vivo: false, podeExplorar: false). O objetivo é usar o JavaScript para simular essa exploração e mostrar como o explorador muda a cada planeta visitado.
...
`,
    ...
    inputs: [
      {
        name: "explorador",
        type: "textarea",
        label: "Digite o explorador inicial (JSON):",
      },
    ...
    solution: function (inputs) {
      try {
        // Se exploradorState for nulo, inicialize-o com o JSON de entrada
        if (!window.exploradorState) {
          window.exploradorState = JSON.parse(inputs.explorador);
        }
        ...
        window.exploradorState.planetasConhecidos.push(planeta);

        // Verifica falha crítica
        if (dado1 === 1 && dado2 === 1) {
          window.exploradorState.vivo = false;
          window.exploradorState.podeExplorar = false;
        } else {
          // Sucesso na exploração
          window.exploradorState.experiencia += 25;

          // Verifica se subiu de nível
          if (
            window.exploradorState.experiencia >= 1365 &&
            window.exploradorState.nivel < 10
          ) {
            window.exploradorState.nivel = 10;
          }

          // Verifica ranque
          if (window.exploradorState.nivel >= 10) {
            window.exploradorState.ranque = "Explorador";
          }

          // Verifica se se tornou especialista no terreno
          const terreno = planeta.terrain;
          if (
            window.exploradorState.especialidades[terreno] &&
            window.exploradorState.especialidades[terreno].acertosCriticos >= 2 &&
            !window.exploradorState.especialidades[terreno].bonus
          ) {
            window.exploradorState.especialidades[terreno].bonus = 1;
          }
        }

        // Formata a saída para exibir as propriedades do explorador
        return `
                Nível: ${window.exploradorState.nivel}
                Experiência: ${window.exploradorState.experiencia}
                Ranque: ${window.exploradorState.ranque}
                Planetas Conhecidos: ${window.exploradorState.planetasConhecidos
                  .map((p) => p.name)
                  .join(", ")}
                Vivo: ${window.exploradorState.vivo}
                Pode Explorar: ${window.exploradorState.podeExplorar}
            `;
      } catch (e) {
  ...
  ...Array.from({}, (_, i) => ({
    ...
    solution: function (inputs) {
      return `Resultado para Desafio Difícil ${i + 3}: ${
        inputs[`inputD${i}`] || "vazio"
      }`;
    },
  })),
    ...
    inputs: [
      {
        ...
        type: "textarea",
        label:
          'Digite a matriz do labirinto (ex: [[" ", "S", " "], ["#", " ", "E"]]):',
      },
    ],
    solution: function (inputs) {
      try {
...
- Escrever valores em células (ex: "write A1 10").
- Ler valores de células, incluindo o resultado de fórmulas (ex: "read A1").
- Salvar o estado da planilha em formato JSON (ex: "save").
- Carregar o estado da planilha a partir de um JSON (ex: "load {\"A1\": \"10\"}").

As fórmulas devem seguir o formato "FORMULA(OPERACAO(CEL1;CEL2;...))", onde OPERACAO pode ser:

- SUM: Soma os valores das células.
- SUB: Subtrai os valores das células.
- MUL: Multiplica os valores das células.
- DIV: Divide os valores das células.
- MIN: Retorna o valor mínimo das células.
- MAX: Retorna o valor máximo das células.
- AVG: Calcula a média dos valores das células.
...
A1: 10
B1: 20
C1: FORMULA(SUM(A1;B1))
Resultados:
30
{"A1":"10","B1":"20","C1":"FORMULA(SUM(A1;B1))"}
\`\`\`
`,
    difficulty: "very-hard",
    inputs: [
      {
        name: "comandos",
        type: "textarea",
        label: "Digite os comandos da planilha (um por linha):",
      },
    ],
    solution: function (inputs) {
      try {
        const linhas = inputs.comandos
          .trim()
          .split("\n")
          .map((l) => l.trim())
          .filter(Boolean);
        ...
        // Se planilhaState for nulo, inicialize-o
        if (!window.planilhaState) {
          window.planilhaState = new Spreadsheet();
        }

        const planilha = window.planilhaState;
        const resultados = [];
        ...
        // Formata a saída para exibir o estado da planilha
        let output = "Estado da Planilha:\n";
        for (const celula in planilha.data) {
          output += `${celula}: ${planilha.data[celula]}\n`;
        }
        output += `Resultados:\n${resultados.join("\n")}`;
  ...
  ...Array.from({}, (_, i) => ({
    ...
    solution: function (inputs) {
      return `Resultado para Muito Difícil ${i + 1}: ${
        inputs[`inputVH${i}`] || "vazio"
      }`;
    },
  })),
];